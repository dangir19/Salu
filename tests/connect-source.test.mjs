...
test("wires Connect APIs, provider payouts UI, and D1 tables", () => {
  ...
  assert.match(workspace, /\/api\/connect\/me/);
  assert.match(workspace, /\/api\/connect\/onboard/);
  assert.match(workspace, /Set up payouts/);
  assert.match(workspace, /Payouts enabled/);
  assert.match(workspace, /Not connected/);
  assert.match(workspace, /Open payouts dashboard/);
