...
- Provider and admin calendars stay prototype / local.
- Availability inventory is the Miami catalog (`domain/mock-data.ts`), not a held-slot table.
- Stripe Connect / `ProviderPayout` is still the next payments step ([NEXT_PAYMENTS.md](./NEXT_PAYMENTS.md)).
- Atlas LLM, Cloudflare DNS cutover, and live credential review are unchanged.
